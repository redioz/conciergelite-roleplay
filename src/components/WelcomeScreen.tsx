...
function UsageMeter({ secondsRemaining, loading }: { secondsRemaining: number; loading: boolean }) {
  const totalSeconds = 3600;
  const used = totalSeconds - secondsRemaining;
  const percent = Math.min(100, (used / totalSeconds) * 100);
  const minutesLeft = Math.ceil(secondsRemaining / 60);

  if (loading) {
    return (
      <div className="flex items-center gap-2 px-3 py-1.5 glass rounded-xl">
        <div className="w-3 h-3 rounded-full border-2 border-gold/30 border-t-gold animate-spin" />
        <span className="text-xs text-text-muted">...</span>
      </div>
    );
  }

  const barColor = secondsRemaining > 600 ? 'bg-gold' : secondsRemaining > 120 ? 'bg-orange-400' : 'bg-red-400';
  ...
  return (
    <div className="flex items-center gap-2.5 px-3 py-1.5 glass rounded-xl">
      ...
      <span className={`text-xs font-medium ${secondsRemaining <= 120 ? 'text-red-400' : 'text-text-muted'}`}>
        {minutesLeft}min
      </span>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="glass sticky top-0 z-30 flex items-center justify-between px-6 py-4 rounded-none border-t-0 border-x-0">
        ...
        <div className="flex items-center gap-2">
          {/* Usage meter */}
          <UsageMeter secondsRemaining={secondsRemaining} loading={usageLoading} />
          ...
          <button
            onClick={onSignOut}
            className="p-2.5 rounded-xl glass hover:bg-white/[0.07] text-text-muted hover:text-red-400 transition-all"
            title="Déconnexion"
          >
            <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
